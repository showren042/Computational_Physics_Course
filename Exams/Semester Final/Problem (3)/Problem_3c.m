...
vals=[-25,-145,85000,10500,-175];  %initial guess.

bestfit=fminsearch(@(x)lsquares(t,ave./nrun,@fitfunction,x),vals);  

figure
plot(t,fitfunction(t,bestfit),'ro'); hold on;
plot(t,ave./nrun,'b-','Linewidth',1.5);
xlabel('n');
ylabel('<E_n>');
legend('fitted','numerical')
...
function x=fitfunction(t,params)
    ...
    x=A_1*exp(-t./tau_1)+ A_2*exp(-t./tau_2)+B;  %our fit function
end
