   ...
   for step=1:nstep
      ...
      if (step-t_e > 0)
          ...
          if(mod((step-t_e),printstep)==0)
              ...
              t(k)=step-t_e;
              mcorr(k)=mcorr(k)+abs(m)*abs(m_eq)/N/N;
          end
      end
   end
end

vals=[-300,10000];  %initial guess.

bestfit=fminsearch(@(x)lsquares(t,(mcorr./nrun),@fitfunction,x),vals);  

figure
plot(t,fitfunction(t,bestfit),'ro'); hold on;
plot(t,mcorr./nrun,'b-','Linewidth',1.5);
title('magnetization correlation')
xlabel('iteration')
ylabel('$<m_{\tau_{e}}m_{\tau_{e} + n}>$','interpreter','latex')
...
function x=fitfunction(t,params)
    ...
    x=D*exp(-t./tau_c);  %our fit function
end
